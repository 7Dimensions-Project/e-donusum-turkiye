# -*- coding: utf-8 -*-
"""
Odoo Online (SaaS) Server Action: ⚡ e-Fatura: Kabul Et (GİB)
Model: account.move
...
for record in records:
    ...
    # 2. Senaryo Kontrolü (Temel faturalar GİB üzerinden kabul/reddedilemez)
    ...
    # Fallback to config parameter if field doesn't exist
    if not api_key:
    ...
    # 4. Nilvera SendAnswer API Çağrısı
    ...
    if r.status_code in (200, 201, 202, 204):
        record.sudo().write({
            'x_parasut_status': 'accepted',
            'x_parasut_answer_note': 'GİB üzerinden kabul edildi.'
        })
        record.message_post(body="⚡ <b>GİB Uygulama Yanıtı:</b> Fatura Nilvera/GİB üzerinden başarıyla <b>KABUL</b> edildi.")
    elif r.status_code == 409:
        # Zaten cevaplanmış
        record.sudo().write({
            'x_parasut_status': 'accepted'
        })
        record.message_post(body="ℹ️ <b>GİB Durumu:</b> Fatura Nilvera portalında zaten kabul edilmiş görünüyor. Odoo durumu güncellendi.")
    else:
        err_msg = "Nilvera Hata Yanıtı [" + str(r.status_code) + "]: " + r.text
        raise UserError("GİB Kabul Gönderilemedi!\n\n" + err_msg)
