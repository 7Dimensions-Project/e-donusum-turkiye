# -*- coding: utf-8 -*-
"""
Wizard to manually fetch Nilvera E-Dönüşüm documents for a specific date range.
"""
...
class FetchInvoicesWizard(models.TransientModel):
    ...
    _description = 'Nilvera Fatura Çekme Sihirbazı'
    ...
    def action_fetch_invoices(self):
        """Executes document fetch from Nilvera."""
        self.ensure_one()
        ...
        try:
            if self.doc_type == 'inbound_invoice':
                page = 1
                while True:
                    res = client.get_inbound_invoices(start_date=start_str, end_date=end_str, page=page, page_size=50)
                    ...
                    for item in items:
                        ...
                        existing = self.env['account.move'].search([
                            ...
                            '|', ('x_edonusum_uuid', '=', uuid), ('x_parasut_uuid', '=', uuid)
                        ], limit=1)

                        if existing:
                            # Update status
                            st = item.get('Status') or item.get('InvoiceStatus')
                            if st:
                        ...
                        # Allowances & Charges (Yuvarlama farkı vb.)
            ...
            return {
                ...
                'params': {
                    'title': _("Fatura Çekme Tamamlandı"),
